...
import { ToolCategories, ToolAnnotations } from './categories.js';
...
export const pdfSaveTool = {
  ...
  parameters: z.object({
    ...
    headerTemplate: z
      ...
      .optional()
      .default(
        '<div style="font-size:10px; width:100%; text-align:center;"><span class="date"></span></div>'
      )
    ...
    footerTemplate: z
      ...
      .optional()
      .default(
        '<div style="font-size:10px; width:100%; text-align:center;">Page <span class="pageNumber"></span> sur <span class="totalPages"></span></div>'
      )
...
export const pdfPrintMultiplePagesTool = {
  ...
  execute: async (args: any, _context: Context<AuthData>) => {
    ...
    const validPages = pageIds.filter((id: string) => pages.has(id));
