    ...
    try {
      ...
      if (viewport) {
        if (x < 0 || x > viewport.width || y < 0 || y > viewport.height) {
          throw new Error(
            `Coordonnées hors limites. Page: ${viewport.width}x${viewport.height}, Reçu: ${x},${y}`
          );
        }
      }
...
export const mouseDragXyTool = {
  ...
  execute: async (args: any, _context: Context<AuthData>) => {
    ...
    try {
      ...
      await page.mouse.down({ button: button as 'left' | 'middle' | 'right' });
      ...
      // Relâcher le bouton
      await page.mouse.up({ button: button as 'left' | 'middle' | 'right' });
  ...
  execute: async (args: any, _context: Context<AuthData>) => {
    ...
    try {
      ...
      if (selector) {
        ...
        elements = await page.evaluate((searchText: string) => {
          const results: any[] = [];
