...
describe('Server Tests', () => {
    ...
    beforeEach(() => {
        ...
        loggerMock.child.mockReturnValue(mockChildLogger);
    ...
    describe('authHandler', () => {
        it('should authenticate successfully', async () => {
            ...
            const authData = await authHandler(req);
            expect(authData).toBeDefined();
            expect(loggerMock.child({}).info).toHaveBeenCalledWith({ authId: 'mock-uuid' }, 'Authentification réussie.');
        });
        it('should fail for missing auth header', async () => {
            const req = { headers: {}, socket: { remoteAddress: '127.0.0.1' } };
            await expect(authHandler(req)).rejects.toThrow('Accès non autorisé');
            expect(loggerMock.child({}).warn).toHaveBeenCalledWith({ clientIp: '127.0.0.1' }, "Tentative d'accès non autorisé: en-tête 'Authorization' manquant ou invalide.");
        });
    });
    describe('applicationEntryPoint', () => {
        ...
        it('should handle SIGTERM', async () => {
            ...
            if (!sigtermHandler)
                throw new Error('sigtermHandler not defined');
