...
function generateSignature(payload) {
    ...
    if (!secret) {
        logger.error(`[WebhookUtils] ${WEBHOOK_SECRET_ENV_VAR} n'est pas défini. Impossible de signer le webhook.`);
 ...
 */
export async function sendWebhook(url, payload, taskId, toolName, throwErr = false) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            log.error({ status: res.status, statusText: res.statusText, body: errBody }, 'Le webhook a échoué avec une réponse non-OK.');
            if (throwErr) {
                throw new WebhookError(`La livraison du webhook à ${url} a échoué. Statut : ${res.status} ${res.statusText}`, 'WebhookDeliveryError', res.status, errBody, { originalPayload: payload });
            }
            return false;
        }
        ...
        if (throwErr) {
            if (error instanceof WebhookError)
                throw error;
            throw new WebhookError(`Erreur d'infrastructure lors de l'envoi du webhook à ${url}: ${errDetails.message}`, // Now uses errDetails.message
            'WebhookInfrastructureError', undefined, undefined, errDetails);
        }
        return false;
    }
}
...
export function verifyWebhookSignature(payload, receivedSignature, secret) {
    if (!payload || !receivedSignature || !secret) {
        logger.warn('[WebhookUtils] Vérification de signature impossible : payload, signature ou secret manquant.');
        return false;
    }
    try {
        const hmac = crypto.createHmac('sha256', secret);
        ...
        logger.error({ err: getErrDetails(error) }, '[WebhookUtils] Erreur lors de la vérification de la signature du webhook.');
        return false;
    }
}
