...
export function isValidHttpUrl(urlString, context) {
    ...
    try {
        ...
        if (url.protocol !== 'http:' && url.protocol !== 'https:') {
            if (context) {
                logger.warn({ context, url: urlString, reason: 'Protocole non supporté' }, "Tentative d'utilisation d'une URL avec un protocole non HTTP/HTTPS.");
            }
            return false;
        }
        return true;
        ...
        if (context) {
            logger.warn({ context, url: urlString, error: e.message }, "Format d'URL invalide détecté.");
        }
        return false;
    }
}
