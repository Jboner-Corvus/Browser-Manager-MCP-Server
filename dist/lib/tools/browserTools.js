...
export const connectExternalBrowserTool = {
    ...
    parameters: z.object({
        ...
        tabId: z
            .string()
            .optional()
            .describe("ID spécifique de l'onglet à contrôler (optionnel, prend le premier onglet si non spécifié)"),
    }),
    execute: async (args, _context) => {
        ...
        try {
            ...
            if (tabId) {
                ...
                selectedTab = tabs.find((tab) => tab.type === 'page') || tabs[0];
            }
            ...
            if (!browser) {
                throw new Error(`Impossible de se connecter après ${maxRetries} tentatives. Dernière erreur: ${lastError?.message}`);
            }
